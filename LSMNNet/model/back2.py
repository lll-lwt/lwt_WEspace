...
class GlobalLocalAttention(nn.Module):
    def __init__(self,
        ...
        # Assuming Conv, ConvBN, SeparableConvBN are defined elsewhere
        self.qkv = Conv(dim, 3 * dim, kernel_size=1, bias=qkv_bias)
        self.local1 = ConvBN(dim, dim, kernel_size=3)
        self.local2 = ConvBN(dim, dim, kernel_size=1)
        self.proj = SeparableConvBN(dim, dim, kernel_size=window_size)

        self.attn_x = nn.AvgPool2d(kernel_size=(window_size, 1), stride=1, padding=(0, window_size // 2))
        self.attn_y = nn.AvgPool2d(kernel_size=(1, window_size), stride=1, padding=(window_size // 2, 0))
...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
                                          out_indices=(1, 2, 3, 4), pretrained=pretrained)
        self.conv1 = nn.Conv2d(48, 64, kernel_size=7, stride=2, padding=3)  # 调整卷积层的输入通道数
        self.bn1 = self.backbone.bn1
        self.act1 = self.backbone.act1
        self.maxpool = self.backbone.maxpool
        self.layers = nn.ModuleList([self.backbone.layer1,
