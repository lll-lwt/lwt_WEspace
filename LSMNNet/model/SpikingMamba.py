...
#
#         return out
class GlobalLocalAttentionWithPEM_CA(nn.Module):
    def __init__(self,
                 dim=256,
                 num_heads=16,
                 qkv_bias=False,
        ...
        # Original components of GlobalLocalAttention
        self.qkv = Conv(dim, 3 * dim, kernel_size=1, bias=qkv_bias)
        self.local1 = ConvBN(dim, dim, kernel_size=3)
        self.local2 = ConvBN(dim, dim, kernel_size=1)
        self.proj = SeparableConvBN(dim, dim, kernel_size=window_size)
...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=16, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = GlobalLocalAttentionWithPEM_CA(dim, num_heads=num_heads, qkv_bias=qkv_bias, window_size=window_size)
...
class FusionAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # local = self.local2(y) + self.local1(y)
        local1_out = self.local1(y)
        local2_out = self.local2(y)
        # 调整 local1_out 的形状，使其与 local2_out 的形状匹配
        local1_out = F.interpolate(local1_out, size=local2_out.shape[2:], mode='bilinear', align_corners=False)
        local = local1_out + local2_out
        x = self.pad(x, self.ws)
        B, C, Hp, Wp = x.shape
        qkv = self.qkv(x)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, window_size):
        ...
        # Implementation of the block, assuming it is a transformer block with a window mechanism
...
class WF(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(WF, self).__init__()
        # Implementation of the weighted fusion (WF) module

    def forward(self, x, res):
        # Forward pass of the weighted fusion module
        return x

class FeatureRefinementHead(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(FeatureRefinementHead, self).__init__()
        # Implementation of the feature refinement head

    def forward(self, x, res):
        # Forward pass of the feature refinement head
        return x
class SimpleConfig:
    def __init__(self, num_classes):
        self.MODEL = self.ModelConfig(num_classes)

    class ModelConfig:
        def __init__(self, num_classes):
            self.MASK_FORMER = self.MaskFormerConfig(num_classes)

        class MaskFormerConfig:
            def __init__(self, num_classes):
                self.NUM_OBJECT_QUERIES = num_classes  # 根据需要设置其他配置参数
...
class LocalRepresentation(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn(x)
        print(f"BatchNorm output shape: {x.shape}")

        x_3x3 = self.to_query_3x3(x)
        print(f"3x3 Conv output shape: {x_3x3.shape}")
...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        self.Fuse = nn.ModuleList()
        self.mask_former_decoder = MaskFormerDecoder(encoder_channels, decode_channels, dropout, window_size,
                                                     num_classes)
        self.local_rep_layers = nn.ModuleList([LocalRepresentation(d_model=ssm_dims[i]) for i in range(4)])  # Initialize LocalRepresentation modules
        for i in range(4):
            fuse = FusionBlock(encoder_channels[i], ssm_dims[i])
            self.Fuse.append(fuse)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)):
            x = self.layers[i](x)
            x = self.Fuse[i](x, vss_outs[i + 1])
            # x = self.local_rep_layers[i](x)  # Apply local representation at each layer
            res = x
            ress.append(res)

        x = self.mask_former_decoder(ress[0], ress[1], ress[2], ress[3], h, w)
        return x, torch.sigmoid(x)
