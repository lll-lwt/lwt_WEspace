...
from model.heads.head_cmtsk_3df import *
from model.ptavitssg2.ptavitssg2_dn_features import *


# Conditioned multitasking. 
class ptavitssg2_dn_cmtsk(torch.nn.Module):
    def __init__(self, in_channels, NClasses,  nfilters_init=96, nheads_start=96//4, depths=[2,2,5,2], spatial_size_init=512, verbose=True, norm_type='GroupNorm', norm_groups=4, correlation_method='linear',segm_act='sigmoid', nblocks3D=2):
        super().__init__()
    ...
    def forward(self,input_t1, input_t2):
        ...
        return self.head(lst_out), torch.sigmoid(self.head(lst_out))
